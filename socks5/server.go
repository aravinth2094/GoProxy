...
func (c *Conn) commandConnect(cmd *cmd) error {
	...
	go func() {
		...
		//c.server.Logger.Println(n, "bytes written from", conn.RemoteAddr().String(), "to", c.rwc.RemoteAddr().String())
	}()
	go func() {
		defer wg.Done()
		//var n int64
		_, err2 = io.Copy(conn, c.rwc)
		//c.server.Logger.Println(n, "bytes written from", c.rwc.RemoteAddr().String(), "to", conn.RemoteAddr().String())
	}()
	wg.Wait()
